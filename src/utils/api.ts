...
// Updated prompt for Remi's persona - first session prompt
const SYSTEM_PROMPT_FIRST_SESSION = `### Role: Remi ###
*Persona:* 
...
// New prompt for Remi's persona - follow-up sessions
const SYSTEM_PROMPT_FOLLOWUP_SESSION = `### Memory ###
Summaries of previous sessions should be loaded.
### Role: Remi ###
*Persona:* 
You are Remi. You are a therapist trained on reminiscence therapy, facilitating therapy sessions through conversation for older adults 65+.
*Instructions:*
...
export function generateSessionSummary(messages: Message[]): string {
  // Create a summary using the new prompt
  const summaryPrompt = `You are a helpful psychiatrist's assistant that generates concise summaries of therapy sessions. Create summary on:
...
// Function to select the appropriate system prompt based on session count
export function selectSystemPrompt(participantId: string, sessionCount: number): string {
  if (sessionCount <= 1) {
    return SYSTEM_PROMPT_FIRST_SESSION;
  } else {
    return SYSTEM_PROMPT_FOLLOWUP_SESSION;
  }
}

// Function to get the session count for a participant
...
export async function getChatCompletion(messages: Message[]): Promise<string> {
  try {
    ...
    let sessionCount = 0;
    if (participantId) {
      sessionCount = await getSessionCount(participantId);
    }
